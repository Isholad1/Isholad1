...
winter_data[is.na(winter_data)] <- mean(winter_data)
forest_model <- randomForest(as.formula(paste(outcome_var, "~ .")), data = winter_data[, model_vars], method = "anova")

# Make predictions using the random forest model
predictions <- predict(forest_model, winter_data[, model_vars])
...
varImpPlot(forest_model, main = "Variable Importance")
varImpPlot(forest_model, main = "Variable Importance", col = c("red", "blue", "black"))

#PLOT on Forest
plot(winter_data$predicted_productivity, winter_data$productivity, main = "Predicted vs. Actual Productivity",
     xlab = "Predicted Productivity", ylab = "Actual Productivity")
...
winter_data %>%
  ...
  labs(
    title = "Figure 4.2",
    subtitle = "Density of Gender by Productivity"
  ) +
  theme_linedraw() +
  theme(
    plot.title = element_text(face = "bold"),
...
winter_data %>%
  ...
  ggplot() +
  aes(x = Productivity, fill = Gender) +
  geom_histogram(position = "identity", alpha = 0.9, stat = "bin") +
  labs(
    title = "Figure 4.2", subtitle = "Productivity on Gender")
...
winter_data %>%
  ...
  filter(!is.na(Age)) %>%
  ggplot() +
  aes(x = Age, y = Home_based_working, xend = lead(Age), yend = Home_based_working) +
  geom_segment(color = "#112446", size = 1, lty = 2) +
...
winter_data %>%
  ...
  filter(!is.na(Age)) %>%
  ggplot() +
  aes(x = Age, y = Home_based_working) +
